...
% tik_lambda_cands_1 =  10.^(linspace(-12,-1,100));
% tik_lambda_cands_2 =  10.^(linspace(-16,-4,200));
tik_lambda_cands_1 =  10.^(linspace(-8,-1,200));
tik_lambda_cands_2 =  10.^(linspace(-16,-7,200));

plotting = false;


precond_type = 'robust' %'robust'% 'approx','exact' or 'robust'
sample_sizes = [40:20:200] %round(linspace(5,P,10));x % It takes over an hour to precondition a 1000x20 system!!!!!
sample_inc = 20
...
gradient_type = 'tik' % TV, tik, or exact
...
% Mu_WLASSO = zeros(R,1);
% Mu_WLASSO_av = zeros(R,1);
% cond_WLASSOD = zeros(R,1);
% norm_WLASSO = zeros(R,1);
% fro_norm_W_LASSO = zeros(R,1);
% inf_norm_WLASSO = zeros(R,1);
% cond_WLASSO = zeros(R,1);
% lambda_WLASSO = zeros(R,1);
...
% LASSO_err = zeros(R,1);
% LASSO_model_err = zeros(R,1);
% LASSO_stats = zeros(num_samps,12);
% LASSO_fit = zeros(R,1);

% WLASSO_err = zeros(R,1);
% WLASSO_model_err = zeros(R,1);
% WLASSO_stats = zeros(num_samps,15);
% WLASSO_fit = zeros(R,1);
...
%             toc(timerval)
        ...
        if strcmp(gradient_type,'tik')
            ...
            V = [dxdt, dydt];
            plot(1:NN,V_RRef(:,1),1:NN,V_RRef(:,2));
            hold on
            plot(1:NN,V(:,1),1:NN,V(:,2));
            drawnow
            dx1_err = norm(V(:,1)-V_RRef(:,1))/norm(V_RRef(:,1));
            dx2_err = norm(V(:,2)-V_RRef(:,2))/norm(V_RRef(:,2));
            dV = V - V_RRef;
            FD_approx_error = mean([dx1_err, dx2_err])
            fprintf(' FD err =  %f, dxdt err = %f, dydt err = %f\n', FD_approx_error,  dx1_err ,  dx2_err  )
            %toc(timerval)
            %return
        end
...
%             fprintf('WLASSO_err = %f \n',WLASSO_model_err(r));
        ...
        fprintf('Iteration r = %i on ppid %i, took  %g seconds, and used %g (MB) \n', r,str2double(ppid), toc(timerval) ,str2double(memory_used)*10^(-3));
